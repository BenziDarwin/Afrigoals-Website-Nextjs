...
import { mockLeagues, mockClubs, mockUsers, mockNewsArticles } from '@/lib/mock-api';
...
export default function AdminDashboard() {
  ...
  const statCards = [
    { title: 'Total Leagues', value: stats.leagues, icon: Trophy, color: 'text-blue-500' },
    { title: 'Total Clubs', value: stats.clubs, icon: Building2, color: 'text-green-500' },
    { title: 'Total Users', value: stats.users, icon: Users, color: 'text-purple-500' },
    { title: 'Published Articles', value: stats.articles, icon: Newspaper, color: 'text-orange-500' },
  ];
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-neutral-400">Manage all aspects of the Afrigoals platform</p>
        </div>
            ...
            <div className="space-y-4">
              {[...Array(5)].map((_, i) => (
                <div key={i} className="flex items-center space-x-3 text-neutral-400">
                  ...
                  <span className="text-sm">System activity log item {i + 1}</span>
                </div>
              ))}
            </div>
          </Card>
