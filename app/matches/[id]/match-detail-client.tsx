...
export default function MatchDetailClient({ matchId }: { matchId: string }) {
  ...
  const homeFormation = mockFormations.find(f => f.match_id === matchId && f.club_id === homeClub?.id);
  const awayFormation = mockFormations.find(f => f.match_id === matchId && f.club_id === awayClub?.id);
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'live':
        return <Badge className="bg-red-500 animate-pulse text-lg px-4 py-1">LIVE</Badge>;
      case 'completed':
        return <Badge className="bg-green-600 text-lg px-4 py-1">FULL TIME</Badge>;
      case 'scheduled':
        return <Badge variant="outline" className="border-neutral-600 text-neutral-400 text-lg px-4 py-1">UPCOMING</Badge>;
      default:
        return null;
    }
  };
            ...
            <div className="grid grid-cols-3 gap-8 items-center max-w-4xl mx-auto">
              <div className="text-center">
                <div className="w-24 h-24 mx-auto mb-4 rounded-full bg-neutral-800 flex items-center justify-center">
                  <span className="text-3xl font-bold text-primary-400">{homeClub.name.charAt(0)}</span>
                </div>
                <h2 className="text-2xl font-bold text-white">{homeClub.name}</h2>
              </div>
              ...
              <div className="text-center">
                <div className="w-24 h-24 mx-auto mb-4 rounded-full bg-neutral-800 flex items-center justify-center">
                  <span className="text-3xl font-bold text-primary-400">{awayClub.name.charAt(0)}</span>
                </div>
                <h2 className="text-2xl font-bold text-white">{awayClub.name}</h2>
              </div>
            </div>
          </Card>
